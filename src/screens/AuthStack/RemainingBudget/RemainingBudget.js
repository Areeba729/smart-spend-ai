...
const RemainingBudget = ({ navigation, route }) => {
  ...
  return (
    <View style={styles.container}>
      <SimpleHeader title="Remaining" onBackPress={() => navigation.goBack()} />
      {loading ? (
        <ScreenLoader />
      ) : (
        <>
          <View style={styles.summarySection}>
            ...
            <View style={styles.statsRow}>
              <View style={styles.statBox}>
                <NativeText style={styles.statLabel}>Budget</NativeText>
                <NativeText style={styles.statValue}>
                  {(monthlyBudget || 0).toFixed(0)}
                </NativeText>
              </View>
              <View style={styles.statBox}>
          ...
          >
            {expenses.length > 0 ? (
              expenses.map((expense, index) => (
                <ExpenseItem
                  key={expense.id || `remaining-${index}`}
                  icon={expense.icon || '💸'}
